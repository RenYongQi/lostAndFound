...
$userInputValue = trim(htmlspecialchars($_POST['userInputValue']));
$query = $pdo->query("select a.name,a.place,a.date,a.state,b.picsrc,c.usericon,c.username,c.usertel from tb_wx_fb as a left join tb_wx_fb_pic as b on a.id = b.picid left join tb_wx_Login as c on a.codeid = c.codeid where a.name like '%$userInputValue%' order by a.date desc ");
